...
const generateSmartMix = (config: LotteryConfig, prevBets: any[]) => {
  const getSmartSet = (max: number, count: number, allowDup: boolean, zeroBased: boolean, prevNumbers: number[][]) => {
    if (count === 0) return [];
  ...
  const reds = getSmartSet(config.red.max, config.red.count, !!config.red.allowDuplicate, zeroRed, prevReds);
  const blues = getSmartSet(config.blue.max, config.blue.count, !!config.blue.allowDuplicate, zeroBlue, prevBlues);
...
const ResultCard: React.FC<{ lottery: LotteryConfig, result: any, hideLotteryInfo?: boolean, isWinner?: boolean }> = ({ lottery, result, hideLotteryInfo, isWinner }) => {
  ...
  return (
    <div className={`bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-white/60 dark:border-slate-800/60 rounded-2xl p-4 sm:p-5 w-full flex ${isSmallLottery ? 'flex-row items-center justify-between flex-wrap gap-4' : 'flex-col gap-4'} shadow-[0_4px_16px_rgba(0,0,0,0.04)] relative overflow-hidden`}>
      ...
      {lottery.id === 'KL8' ? (
        /* KL8 special layout: 20 balls in a compact 10x2 grid */
        <div className="w-full">
          <div className="flex flex-wrap gap-[5px] justify-start">
            {result.reds.map((n: number, i: number) => (
              <div key={`r-${i}`} className="relative w-[30px] h-[30px] bg-white dark:bg-slate-800 rounded-full shadow-[0_2px_4px_rgba(0,0,0,0.1)] p-[2px] flex-shrink-0">
                <div className="w-full h-full rounded-full text-white flex items-center justify-center font-bold text-[11px]" style={{ backgroundImage: `url(${getBallImage('red', lottery.id)})`, backgroundSize: '102%', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
                  <span style={{ textShadow: '0 1px 1px rgba(0,0,0,0.5)' }}>{formatNum(n, lottery.red.max)}</span>
                </div>
              </div>
            ))}
          </div>
...
const PickView = ({ selectedLotteryId, onSelectLottery, onSave, resultsData }: { selectedLotteryId: LotteryId, onSelectLottery: (id: LotteryId) => void, onSave: (id: LotteryId, sets: any[], multiplier?: number, isDltExtra?: boolean) => void, resultsData: Record<string, any[]> }) => {
  ...
  const handleGenerate = (count: number) => {
    ...
    setTimeout(() => {
      ...
      if (count === 1 && sets.length >= 4) {
        // If clicking 1 bet and we already have 4+ bets, use the previous 4 bets as smart context
        const prev4 = sets.slice(0, 4);
        newSets.push(generateSmartMix(config, prev4));
      } else {
        for (let i = 0; i < count; i++) {
          // If we are generating multiple and reach the 5th bet
      ...
      setSets(prev => [...newSets, ...prev].slice(0, 50)); // Allow holding more bets to see the progression
  ...
  return (
    <div className="flex flex-col h-full bg-transparent dark:bg-transparent relative">
      ...
      <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md pt-[calc(env(safe-area-inset-top,44px)+6px)] pb-2 px-4 shadow-[0_2px_10px_rgba(0,0,0,0.02)] z-10 sticky top-0 border-b border-white/20 dark:border-slate-800">
          ...
          {LOTTERIES.map(l => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1 transition-all outline-none flex-shrink-0 ${isSelected ? 'scale-[1.05] origin-bottom opacity-100' : 'opacity-70 hover:opacity-100 scale-100'}`}
              >
                <div className={`w-[42px] h-[42px] bg-white dark:bg-slate-800 rounded-full p-1.5 transition-all flex items-center justify-center ${isSelected ? 'shadow-[0_4px_12px_rgba(0,0,0,0.1)] ring-2 ring-offset-2 dark:ring-offset-slate-900 ' + (l.theme === 'red' ? 'ring-red-400' : 'ring-blue-400') : 'border border-gray-100 dark:border-slate-800 shadow-sm'}`}>
                  <img src={`.${l.icon}`} alt={l.name} className="w-full h-full object-contain drop-shadow-sm" />
                </div>
                <span className={`text-[11px] whitespace-nowrap font-bold transition-all ${isSelected ? (l.theme === 'red' ? 'text-red-500' : 'text-blue-500') : 'text-gray-500 dark:text-gray-400'}`}>
                   {l.name}
                </span>
              </button>
            );
        ...
        {/* Mode Selector */}
        <div className="flex bg-gray-100 dark:bg-slate-800 rounded-lg p-1 mx-2 mb-0 mt-1">
            ...
            sets.map((set, idx) => (
              <div key={set.id || idx} className="relative w-full overflow-hidden rounded-2xl">
                 ...
                 <motion.div
                   ...
                   className="prevent-swipe bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-2xl py-2 px-3 sm:py-3 sm:px-4 shadow-sm border border-white/60 dark:border-slate-800/60 relative z-10 overflow-hidden flex items-center w-full touch-pan-y"
                 >
                   ...
                   {set.isSmartAppended && (
                     <div className="absolute top-0 right-0 bg-gradient-to-l from-yellow-400 to-amber-500 text-white text-[10px] sm:text-xs font-bold px-2 py-0.5 rounded-bl-[12px] flex items-center shadow-sm">
                       ✨ 算法优选
                     </div>
                   )}
    ...
    <div className="flex flex-col h-full bg-transparent dark:bg-transparent relative overflow-hidden w-full">
      ...
      <div className="flex-1 overflow-y-auto z-10 w-full relative pb-10">
        ...
        <div className="absolute top-[calc(env(safe-area-inset-top,24px)+8px)] right-5 z-[60]">
          ...
          {showSettings && (
            <div className="absolute top-10 right-0 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-100 dark:border-slate-700 p-2 min-w-[160px] animate-in fade-in slide-in-from-top-2">
